...
def any_msg(update: Update, context: CallbackContext):
    ...
    context.bot.sendMessage(recipient, f'''У вас новое анонимное признание ([#]({link}){random_nums()}):\n\n*{update.message.text}*''', parse_mode=telegram.ParseMode.MARKDOWN, disable_web_page_preview=True)
    update.message.reply_text(texts.done().format(update.effective_user.id), parse_mode=telegram.ParseMode.MARKDOWN)

    clear_vars(context)
    print(f'Отправлена валентинка от {link}, кому: {recipient}')
